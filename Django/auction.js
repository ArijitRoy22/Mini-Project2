...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (product) {
        ...
        <div class="Auction">
            ...
            <input id="bidInput" class="bid" type="number" placeholder="">
            <button id="placeBidButton" class="bid-button" onclick='bidding("${product.item_id}")'>Place Bid</button>
            <button id="proceedButton" style="display: none;" onclick="proceedToDelivery()">Proceed Further</button>
        </div>
        `;
                ...
                if (bid) {
                    ...
                    .then(data => {
                        ...
                        .then(itemData => {
                            ...
                            // Update the local storage item to reflect the new start_bid value
                            const updatedProduct = JSON.parse(localStorage.getItem('selectedProduct'));
                            updatedProduct.title7 = data.current_bid;
                            localStorage.setItem('selectedProduct', JSON.stringify(updatedProduct));
                        })
                        .catch(error => {
                            console.error('Error updating item:', error);
                    ...
                    .catch(error => {
                        console.error('Error updating bid:', error);
                        alert('Error updating bid: ' + (error.detail || 'Unknown error'));
                    });
                } else {
                    console.error('Bid not found for this item');
                    alert('Bid not found for this item');
                }
            })
            .catch(error => {
                console.error('Error fetching bids:', error);
                alert('Error fetching bids: ' + (error.detail || 'Unknown error'));
            });
    } else {
